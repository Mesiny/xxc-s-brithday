...
x = setInterval(function () {
  ...
  let w = (c.width = max(window.innerWidth, window.innerHeight)),
    h = (c.height = min(window.innerWidth, window.innerHeight)),
    ctx = c.getContext('2d'),
    hw = w / 2, // half-width
    hh = h / 2,
    opts = {
  ...
  window.addEventListener('resize', function () {
    ...
    w = c.width = max(window.innerWidth, window.innerHeight);
    h = c.height = min(window.innerWidth, window.innerHeight);
